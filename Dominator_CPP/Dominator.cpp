...
{
	unsigned count = 0, candidate_count = 0;
	int candidate = -1, candidate_index = -1;
...
int Dominator2(vector<int> &A)
{
	int leader = -1, candidate_index = -1;
	unsigned count = 0;
	...
	for (unsigned i = 0; i < B.size(); i++)
	{
		if (B[i] == leader)
		{
			candidate_index = i;	// Found the first index of the dominator (leader)
			break;
		}
	}          
...
int main()
{
	int x[8] = { 3, 4, 3, 2, 3, -1, 3, 3 };			// Answer: 6

	vector<int> v(x, x + sizeof(x) / sizeof(x[0]));

	cout << "Solution: " << Dominator(v) << endl;	// Answer: 6
	cout << "Solution: " << Dominator2(v) << endl;	// Answer: 0
